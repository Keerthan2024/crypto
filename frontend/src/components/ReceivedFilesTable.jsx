...
const ReceivedFilesTable = ({ data, loading, error, onPageChange, page, hasNext }) => {
  if (loading) return <div className="text-gray-400 p-4 text-center animate-pulse">Loading incoming transmissions...</div>;
  if (error) return <div className="text-red-400 p-4 text-center">Failed to load received files.</div>;
          ...
          <tbody className="text-sm text-gray-300">
            {data.map((item, idx) => (
              <tr key={idx} className="border-b border-gray-800/50 hover:bg-white/5 transition-colors">
                <td className="p-3 text-cyan-400 font-medium">@{item.sender_username}</td>
                ...
                <td className="p-3 text-right">
                  {item.status === 'Active' ? (
                    <Link to={`/download/${item.share_token}`} className="text-xs font-bold text-cyan-400 hover:text-cyan-300 border border-cyan-500/30 px-3 py-1.5 rounded-lg hover:bg-cyan-500/10 transition-colors">
                      Decrypt
                    </Link>
                  ) : (
                    <span className="text-xs text-gray-600 font-bold uppercase tracking-widest">Locked</span>
                  )}
                </td>
              </tr>
            ))}
